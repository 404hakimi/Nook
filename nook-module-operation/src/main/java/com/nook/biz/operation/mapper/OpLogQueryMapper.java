 ...
 */
@Mapper
public interface OpLogQueryMapper extends BaseMapper<OpLog> {
     ...
     * @return 分页结果
     */
    default IPage<OpLog> selectPageByQuery(IPage<OpLog> page,
                                           OpStatus status,
                                           String serverId,
                                           String opType) {
        return selectPage(page, Wrappers.<OpLog>lambdaQuery()
                .eq(ObjectUtil.isNotNull(status), OpLog::getStatus, status)
                .eq(StrUtil.isNotBlank(serverId), OpLog::getServerId, serverId)
                .eq(StrUtil.isNotBlank(opType), OpLog::getOpType, opType)
                .orderByDesc(OpLog::getEnqueuedAt));
    }
}
