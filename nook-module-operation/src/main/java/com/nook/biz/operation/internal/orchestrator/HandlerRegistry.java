...
/**
 * opType → Handler 的查找表; 启动时由 Spring 把所有 OperationHandler bean 注入到这里.
 *
...
public class HandlerRegistry {
    ...
    public HandlerRegistry(List<OperationHandler> beans) {
        for (OperationHandler h : beans) {
            String type = h.type();
            OperationHandler prev = handlers.put(type, h);
            if (prev != null) {
    ...
    OperationHandler resolve(String type) {
        OperationHandler h = handlers.get(type);
        if (h == null) {
