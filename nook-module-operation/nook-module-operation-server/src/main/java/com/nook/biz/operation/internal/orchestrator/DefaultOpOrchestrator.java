...
import com.nook.biz.operation.api.enums.OpErrorCode;
...
public class DefaultOpOrchestrator implements OpOrchestrator {
    ...
    public <T> T submitAndWait(OpEnqueueRequest req, Duration waitTimeout, Class<T> resultType) {
        ...
        try {
            ...
            throw new BusinessException(CommonErrorCode.INTERNAL_ERROR,
                    "handler 异常: " + (cause == null ? "<unknown>" : cause.getMessage()));
        } finally {
            waiters.remove(opId);
        }
    }
    ...
    private static void validate(OpEnqueueRequest req) {
        if (req == null) throw new BusinessException(CommonErrorCode.PARAM_INVALID, "OpEnqueueRequest 为 null");
        if (req.getServerId() == null || req.getServerId().isBlank())
            throw new BusinessException(CommonErrorCode.PARAM_INVALID, "serverId 为空");
        if (req.getOpType() == null || req.getOpType().isBlank())
            throw new BusinessException(CommonErrorCode.PARAM_INVALID, "opType 为空");
    }
