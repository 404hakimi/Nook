...
type RoleComponents struct {
	...
	Socks5Deploy      control.Socks5DeployFunc // landing: 控制接口 /socks5/deploy 装 dante 实现; nil = 不暴露该端点
...
func Run(version string, registerRole RoleRegister) {
	...
	if cfg.Control.Port > 0 {
		ctrl := control.New(cfg.Control.Port, cfg.Backend.APIToken, comp.XrayDeploy, comp.XrayCert, comp.Socks5Deploy)
		wg.Add(1)
