<script setup lang="ts">
import { ref, watch } from 'vue'
import { RefreshCw, Search } from 'lucide-vue-next'
import {
  NButton,
  NIcon,
  NInput,
  NModal,
  NSelect,
  NSpace,
  NSpin,
  useMessage
} from 'naive-ui'
import { getAgentLog, type AgentLog, type AgentLogLevel } from '@/api/agent/agent'

/**
 * nook-agent systemd journal 查看 dialog.
 *
 * <p>agent stdout/stderr 走 journald (装机时 systemd unit 走默认), 没有独立文件日志.
 */
interface Props {
  modelValue: boolean
  serverId?: string | null
  /** 标题展示用 (落地机名 / 线路机 host 都行). */
  label?: string
}
const props = defineProps<Props>()
const emit = defineEmits<{
  (e: 'update:modelValue', v: boolean): void
}>()

const message = useMessage()

const logLoading = ref(false)
const agentLog = ref<AgentLog | null>(null)

const LOG_LINES_OPTIONS = [
  { label: '50 行', value: 50 },
  { label: '100 行', value: 100 },
  { label: '200 行', value: 200 },
  { label: '500 行', value: 500 },
  { label: '1000 行', value: 1000 }
]
const LOG_LEVEL_OPTIONS: { label: string; value: AgentLogLevel }[] = [
  { label: '全部', value: 'all' },
  { label: '警告以上', value: 'warning' },
  { label: '错误以上', value: 'err' }
]
const logLines = ref(100)
const logLevel = ref<AgentLogLevel>('all')
const logKeyword = ref('')

let keywordTimer: ReturnType<typeof setTimeout> | null = null
function onKeywordChange(v: string) {
  logKeyword.value = v
  if (keywordTimer) clearTimeout(keywordTimer)
  keywordTimer = setTimeout(() => {
    keywordTimer = null
    runLog()
  }, 400)
}

function onKeywordEnter() {
  if (keywordTimer) {
    clearTimeout(keywordTimer)
    keywordTimer = null
  }
  runLog()
}

watch(
  () => [props.modelValue, props.serverId],
  ([open]) => {
    if (open) {
      agentLog.value = null
      runLog()
    }
  }
)

async function runLog() {
  if (!props.serverId || logLoading.value) return
  logLoading.value = true
  try {
    agentLog.value = await getAgentLog(props.serverId, {
      lines: logLines.value,
      level: logLevel.value,
      keyword: logKeyword.value || undefined
    })
  } catch (e) {
    agentLog.value = null
    message.error('拉日志失败: ' + ((e as Error).message ?? ''))
  } finally {
    logLoading.value = false
  }
}

function close() {
  emit('update:modelValue', false)
}
</script>

<template>
  <NModal
    :show="modelValue"
    preset="card"
    style="max-width: 64rem"
    :bordered="false"
    :mask-closable="true"
    @update:show="(v: boolean) => emit('update:modelValue', v)"
  >
    <template #header>
      <span>Agent 日志</span>
    </template>
    <template #header-extra>
      <span v-if="label" class="text-xs text-zinc-500">{{ label }}</span>
    </template>

    <div class="flex gap-2 flex-wrap items-center justify-end mb-3">
      <NInput
        :value="logKeyword"
        size="small"
        class="w-60"
        placeholder="搜关键词 (子串/不区分大小写)"
        clearable
        :disabled="logLoading"
        @update:value="onKeywordChange"
        @keyup.enter="onKeywordEnter"
      >
        <template #prefix><NIcon><Search /></NIcon></template>
      </NInput>
      <NSelect
        v-model:value="logLines"
        :options="LOG_LINES_OPTIONS"
        size="small"
        class="w-24"
        :disabled="logLoading"
        @update:value="runLog"
      />
      <NSelect
        v-model:value="logLevel"
        :options="LOG_LEVEL_OPTIONS"
        size="small"
        class="w-28"
        :disabled="logLoading"
        @update:value="runLog"
      />
      <NButton quaternary size="small" :disabled="logLoading" @click="runLog">
        <template #icon><NIcon><RefreshCw /></NIcon></template>
        刷新日志
      </NButton>
    </div>

    <NSpin :show="logLoading && !agentLog">
      <pre
        class="text-xs max-h-[32rem] overflow-auto bg-zinc-900 text-zinc-100 px-4 py-3 rounded font-mono whitespace-pre-wrap break-all leading-relaxed min-h-32"
      ><code v-if="agentLog?.log">{{ agentLog.log }}</code><span v-else class="text-zinc-500">{{ logLoading ? '拉取中...' : (logKeyword ? `无 &quot;${logKeyword}&quot; 命中` : '点&quot;刷新日志&quot;拉取') }}</span></pre>
    </NSpin>

    <template #footer>
      <NSpace justify="end">
        <NButton size="small" @click="close">关闭</NButton>
      </NSpace>
    </template>
  </NModal>
</template>
