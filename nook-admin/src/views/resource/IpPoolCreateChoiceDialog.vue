 ...
 * IP 池新增 - 方式选择对话框
 *
 * 两种方式 (跟 ResourceIpPoolProvisionModeEnum 对齐):
...
<template>
  ...
  >
    <div class="text-sm text-zinc-500 mb-4">
      根据 IP 的来源选对应方式; 自部署会跑装机脚本搭一套 dante SOCKS5; 第三方录入已有 socks5 凭据.
    </div>
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1">
            ...
            <div class="text-xs text-zinc-500 mt-1">
              我已有第三方 socks5 服务, 只录凭据进池. (provision_mode=2 保留, 功能后期实现)
            </div>
            <ul class="text-xs text-zinc-400 mt-2 list-disc pl-4 space-y-0.5">
              <li>不跑装机脚本</li>
              <li>仅录 host/port/user/pass</li>
              <li>无装机事实子表 (运维操作受限)</li>
            </ul>
          </div>
        </div>
      </NCard>
