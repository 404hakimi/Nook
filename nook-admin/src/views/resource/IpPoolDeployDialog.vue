...
/**
 * 针对已存在 IP 池条目的 SOCKS5 装机 (流式) — 跟 ServerInstallDialog 同模式.
 *
 * - props.ipId: 待装机的 IP 池条目 (lifecycle=INSTALLING 或 LIVE 重装)
 * - 装机前先 GET 详情展示给用户确认 (IP/SOCKS5 端口/SSH host 等)
 * - 用户点"开始装机" → 调 /install-socks5?ipId=xxx → 流式 stdout + lifecycle 自动 LIVE
...
async function onStartInstall() {
  if (!props.ipId || !detail.value) return
  const isReinstall = detail.value.lifecycleState === 'LIVE'
  const ok = await confirm({
    title: isReinstall ? '重装 SOCKS5?' : '开始装机 SOCKS5?',
    message: isReinstall
      ? `IP ${detail.value.ipAddress} 当前已 LIVE; 重装会重新跑 install 脚本, 期间客户端会断流几秒.`
      : `在 ${detail.value.sshHost}:${detail.value.sshPort} 跑 dante 装机脚本; 成功后 lifecycle 切到 LIVE.`,
  ...
  try {
    await installSocks5Stream(props.ipId, appendOutput, abortCtrl.signal)
    deployed.value = true
    message.success('装机完成, lifecycle 已切到 LIVE')
    emit('installed', props.ipId)
  } catch (e) {
    ...
    <NSpin :show="loadingDetail">
      ...
      <div v-else-if="detail">
        ...
        <NDescriptions bordered size="small" label-placement="left" :column="2" class="mb-3">
          ...
          <NDescriptionsItem label="SSH host"><span class="font-mono text-xs">{{ detail.sshHost }}:{{ detail.sshPort }}</span></NDescriptionsItem>
