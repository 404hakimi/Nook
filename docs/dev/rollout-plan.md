...
**本次要补的缺口**(后面每步对应一个):
- 落地机预算无防线: 阈值 100% 触发无冗余 / 售卖不看本月剩余。到顶兜底**定为停售 + 运营补量, 不自动换 IP**(保独占 IP 稳定, 见 landing-budget 闸三)。
...
### 线路机候选组自愈闭环(回应"某环节挂了会不会拖垮链路")

```
   ┌──────────────────────────────────────────────┐
   │                                                │ 回到稳态(组满编)
 ① 运行态 ──→ ② 检测 ────────→ ③ 组内换血 ───┐       │
 客户端连主    job 扫到顶/掉线   坏机移出组      │       │
 cert 一主     findFrontlines   幸存备升主       │       │
 多备          NeedingFailover   补新备到队尾     ↓       │
   ↑  客户端 fallback                         ④ 对账自愈  │
   │  秒切到备(节点已在手)                      每台收敛    │
   │  不依赖后台速度                            agent 增删  │
   └────────── ⑤ 账连续 ←──────────────────────┘         │
 ...
 核心不变量: 切的只是入口(线路机), 出口落地机不变 → 计量零影响;
            任一环挂掉 = 暂停在现状, 恢复后下一轮自动收敛。
 落地机侧: 到顶/掉线**不自动换 IP**(保独占 IP 稳定), 改停售 + 运营补量(landing-budget 闸三)。
```
...
### A2 售卖按"本月剩余预算"准入 ✅(0dff185, 与 A1 同笔: Rules 文件 hunk 交叉)
...
### A3 出站 tag 编入落地机标识(换机收敛的前置) ✅(0a653e4)
...
### A4 落地机到顶 / 掉线自动换机 —— ❌ 已撤销(revert c74db0c)

- **撤销原因**: 独占 IP 的产品价值就是**长期稳定**(账号风控 / 白名单 / 登录态常绑死 IP); 为省流量自动换 IP = 砸招牌, 本末倒置。评审决定**不做自动换机**。
- **改为(无新代码, 现状即满足)**: 落地机到顶 → `markQuotaReached` 置 THROTTLED → 准入网关停接新用户 + 闸二停售该机; **存量用户保留原机原 IP**; 短时小超支由闸一冗余吸收, 运营按告警补流量包 / 调 `usable_percent` / 协调换套餐。掉线 → 停新分配, 存量等恢复。
- **保留**: A3 的 tag 编 ipId 不回退 —— 它服务**运营手动换机**(admin 主动换 IP, 如机房永久收回)的对账收敛, 仍有用。
- **可选后做**: 到顶告警 job(监控持续 THROTTLED / 长 OFFLINE → 通知运营); 不阻塞核心。

**线 A 验收**: 演练「卖(剩余预算准入) → 用 → 到顶(置限流 + 自动停售) → 保 IP 不变 → 账连续」; 与厂商面板对比超支 ≤ 冗余(默认 10%)。

---

## 4. 线 B: 线路机候选组(每步向后兼容: `standby_server_ids` 空 = 完全现状)

### B1 凭证加候选字段 + 查询口径拆分(纯地基, 零行为变化) ✅(cb5200b)
...
### B2 对账下发覆盖备机(可单订阅灰度) ✅(2ac867d, 含删机守卫/重装告警一并切组口径)
...
### B3 开通选主+备 + failover 组维护 ✅(7b09a2e; ⚠ prod failover.enabled 本就 true, 换血逻辑部署即生效)
