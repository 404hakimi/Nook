...
跟 yudao / zqwl 对齐, **所有类** (包括 Controller / Service / ServiceImpl / Validator / Convert / Mapper / DO / VO / framework 基础设施 / Properties / Configuration / 注解 / Resolver / Helper 等) **必须**用多行格式 + `@author nook`. 主体**一行简述职责** (动词或名词短语, 无句号), 不展开实现细节. **禁止**单行 `/** ... */` 类级注释, **禁止**用 `<ul>/<ol>` 列举多分支细节, **禁止**在主体写 how / 踩坑史 / 跨模块叙述.
