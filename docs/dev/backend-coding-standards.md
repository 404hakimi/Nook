...
> 版本: v1.2 ｜ 日期: 2026-05-27 ｜ 适用: 所有 `nook-module-*` 模块
...
理由:
- 外键: 跨服务部署 / 分库分表 / 数据迁移时 FK 成阻碍; 跨表存在性校验放 Validator 更显式 (e.g. `landingValidator.validateIpTypeExists`)
...
@GetMapping("/get-host")
Result<HostInfoRespVO> getHost(@RequestParam("id") String id) {
    HostInfoSnapshot snap = fooService.probeHost(id);          // service 漏出 framework 类
    return Result.ok(HostConvert.INSTANCE.convert(snap));      // controller 拼 framework → VO
}
...
public interface XxxOpsService {
    XxxStatusRespVO getStatus(String serverId);
}

// Service Impl: 拉 framework snapshot → Convert 翻译 → 返 VO
@Override
public XxxStatusRespVO getStatus(String serverId) {
    SystemdStatusSnapshot sysd = serverProbe.readSystemdStatus(...);
    HostInfoSnapshot host = serverProbe.readHostInfo(...);
    return XxxOpsConvert.INSTANCE.toStatusRespVO(sysd, host, ...);
}
```
...
public interface LandingSocksOpsConvert {
    ...
    Socks5StatusRespVO toSocks5StatusRespVO(SystemdStatusSnapshot sysd, String ufw, HostInfoSnapshot host);
}
...
public class SystemUserDO extends BaseEntity { ... }
