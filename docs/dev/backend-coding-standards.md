...
- **导读式步骤注释**: 方法体有 **≥2 个不同步骤** (校验 / 取数 / 转换 / 调用 等) 时, 可给每步加单行小标题做快速导读 (e.g. `// 校验服务器是否存在` / `// 转换返回`); 跟「必须加 1」同形, 但**简单 CRUD 方法也允许**, 不必是复杂业务流.
- 取舍由作者定: **带导读的方法 与 不带的单行方法都合规**, 不互相算"不一致"; 唯一要求是同一方法内"要么每步都标、要么都不标", 不半标.

### 禁止加

- 复述代码字面意思 — 指对**单行表达式 / 循环体 / 变量赋值**的逐句翻译 (`// 遍历字段列表`); 与上面「导读式步骤注释」区分: 后者标方法的**业务步骤** (允许), 这里禁的是把一行代码再用中文复述一遍
- 自解释变量的用途说明 (`// 用户 ID`)
- `// 1.` `// 2.` 等序号前缀
- 注释掉的废弃代码 (直接删, git 有历史)
- bug 调查史 / 版本踩坑 (属于 commit message, 不属于代码注释)
- **跨模块叙述**, 如"不下沉到 xxx 模块" — 属于设计文档不属于代码
- **内部机制黑话 / 英文术语** — 用中文业务说法: `provision`→开通, `allocator`→选址, `enrich`→填充/附带, `client`→客户端, `dispatch`→派发; 领域标准词 (SSH / vmess / inbound / tc / 对账 等) 可保留
