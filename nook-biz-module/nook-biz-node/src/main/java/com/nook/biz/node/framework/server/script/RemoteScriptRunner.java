...
public class RemoteScriptRunner {
     ...
     *
     * @param session      已就绪 SSH 会话
     * @param scriptBody   完整脚本内容 (含 shebang / set -e 等头部)
     * @param tmpPrefix    远端 /tmp 文件名前缀
     * @param runTimeout   脚本运行超时
     * @param lineConsumer 每行 stdout 的消费回调
     */
    public void runScriptStreaming(SshSession session,
        ...
        try {
            session.ssh().uploadString(remote, scriptBody, UPLOAD_TIMEOUT);
     ...
     *
     * @param classpath classpath 上的模板路径 (如 "scripts/modules/50-xray.sh.tmpl")
     * @param vars      占位变量表
     * @return 渲染后的脚本字符串
     */
