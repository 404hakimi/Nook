...
public class XrayInstallScriptAssembler {
     ...
     */
    public String assemble(String serverId, XrayInstallReqVO reqVO, InboundProvision prov) {
     ...
     */
    private String assembleModules(XrayInstallReqVO r, InboundProvision prov, Map<String, String> vars) {
        ...
        if (StrUtil.isNotBlank(prov.fullDomain()))   modules.add(NookScripts.MODULE_ACME_TLS);
     ...
     */
    private Map<String, String> buildVars(String serverId, XrayInstallReqVO r, InboundProvision prov) {
        ...
        vars.put("CF_API_TOKEN", useTls ? StrUtil.blankToDefault(prov.cfApiToken(), "") : "");
        ...
        vars.put("SHARED_INBOUND_B64", Base64.getEncoder()
                .encodeToString(prov.inboundJson().getBytes(StandardCharsets.UTF_8)));
        // 部署总结展示用: 协议形态标识 (vmess-ws-tls / vmess-ws-plain / vless-reality)
        vars.put("PROTOCOL_DESC", prov.protocol().getKey());
        return vars;
    }
}
